...
void LButton::swordRender()
{
	...
	swordTexture.render(mPosition.x, mPosition.y, &gSpriteClips[mCurrentSprite]);//was gButtonSpriteSheetTexture
}

void LButton::shieldRender(){
  shieldTexture.render(mPosition.x, mPosition.y, &gSpriteClips[mCurrentSprite]);
}
...
bool loadMedia()
{
	...
	//Load sprites
	if (!swordTexture.loadFromFile("sword.png")) //was gButtonSpriteSheetTexture
	{
		printf("Failed to load sword sprite texture!\n");
		success = false;
	}
  else if(!shieldTexture.loadFromFile("shield.png")){
    printf("Failed to load shield sprite texture!\n");
    success = false;
  }
	else
	{
    ...
    //Load media
        ...
        //Update all windows
			...
			if (allWindowsClosed)
			{
				...
				for (int i = 0; i < TOTAL_BUTTONS; ++i)
				{
          //if item is shield render shield if item is sword render sword
					if(i < 5){
